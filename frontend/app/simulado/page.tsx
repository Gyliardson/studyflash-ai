...
// === CONSTANTES DE DIFICULDADE (ATUALIZADO) ===
const DIFFICULTIES = {
    EASY: { 
        id: 'EASY', 
        label: "Prática", 
...
export default function SimuladoPage() {
    ...
    useEffect(() => {
        Promise.all([listarMeusBaralhos(), listarMeusPlanos()]).then(([d, p]) => {
            ...
            const planParam = searchParams.get('planId'); // Suporte a link direto de plano
    ...
    async function finishExam(finalAnswers: typeof answers) {
        ...
        if (res.success) {
            setFinalResult({ 
                ...
                correctAnswers: correctCount, 
                totalQuestions: examCards.length,
                limitReached: res.limitReached // [NOVO] Captura o aviso de limite
            });
            setStep('RESULT');
        } else {
    ...
    const getSelectionName = () => {
        ...
        if (sourceType === 'PLAN') return planos.find(p => p.id === sourceId)?.title || "Escolher..."; // [NOVO]
        return "...";
    };
                ...
                {step === 'CONFIG' && (
                    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="p-8 space-y-8">
                            {/* 1. Origem: Grid agora é 4 colunas em telas grandes */}
                            ...
                            {/* 2. Dificuldade: Grid agora é 4 colunas */}
                            <div className="space-y-4">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>
                                    2. Intensidade
