...
test("collection mutation server predicates are owner-scoped and repeated deletes fail closed", async () => {
  ...
  try {
    const atomicCreate = await saveFlashcardsForUser(ownerId, [{ frente: "Atomic front", verso: "Atomic back" }], undefined, new Date("2026-08-14T12:00:00Z"), atomicDeckName);
    expect(atomicCreate.success).toBe(true);
    const persistedAtomicDeck = await prisma.deck.findFirst({ where: { userId: ownerId, nome: atomicDeckName }, include: { cards: true } });
    ...
    const duplicateCreate = await saveFlashcardsForUser(ownerId, [{ frente: "Must not persist", verso: "Must not persist" }], undefined, new Date("2026-08-14T12:01:00Z"), atomicDeckName);
...
test("authenticated exam runs through the real server-authoritative attempt boundary", async ({ page }) => {
  ...
  for (let question = 1; question <= 5; question += 1) {
    ...
    if (question < 5) await expect(page.getByText(new RegExp(`${question + 1}\\s*\\/\\s*5`))).toBeVisible();
  }
